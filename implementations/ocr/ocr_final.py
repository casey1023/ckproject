...
import sys
import random
data_dir = r'../../data/ocr_data/'
image_size = 224

epoch = 0

os.makedirs("saved_models/%s%d" % ('ocr', 34), exist_ok=True)
#load data
...
train_loader = []
for i in range(20):
    temp = torch.utils.data.DataLoader(train_dataset, batch_size = 4, shuffle = True, num_workers=8)
...
if had_trained:
    ...
    net.fc = net.fc.cuda() if use_cuda else net.fc
    net.load_state_dict(torch.load("saved_models/%s%d/ocr%d_%d.pth" % ("ocr", 34, 34, trained_num)))
...
for epoch in range(trained_num, num_epochs):
    prev_time=time.time()
    #optimizer = exp_lr_scheduler(optimizer, epoch)
    train_rights = []
    train_losses = []

    which_dataloader = random.randint(0, 19)
    for batch_idx, (data, target) in enumerate(train_loader[which_dataloader]): 
        ...
        #print log
        sys.stdout.write(
            "\r[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [Data Loader: %d]"
            % (
                epoch,
                num_epochs,
                batch_idx,
                len(train_loader[which_dataloader]),
                loss.item(),
                which_dataloader
            )
        )
    ...
    #torch save
    torch.save(net.state_dict(), "saved_models/%s%d/ocr%d_%d.pth" % ('ocr', 34, 34, epoch))
